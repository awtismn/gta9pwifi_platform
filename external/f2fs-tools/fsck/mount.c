...
static struct sb_feature sb_feature_list[] = {
	{ F2FS_FEATURE_ENCRYPT, "encrypt" },
	{ F2FS_FEATURE_VERITY, "verity" },
	{ F2FS_FEATURE_BLKZONED, "blkzoned" },
	{ F2FS_FEATURE_EXTRA_ATTR, "extra_attr" },
	{ F2FS_FEATURE_PRJQUOTA, "project_quota" },
	{ F2FS_FEATURE_INODE_CHKSUM, "inode_checksum" },
	{ F2FS_FEATURE_FLEXIBLE_INLINE_XATTR, "flexible_inline_xattr" },
	{ F2FS_FEATURE_QUOTA_INO, "quota_ino" },
	{ F2FS_FEATURE_INODE_CRTIME, "inode_crtime" },
	{ F2FS_FEATURE_LOST_FOUND, "lost_found" },
	{ F2FS_FEATURE_SB_CHKSUM, "sb_checksum" },
	{ F2FS_FEATURE_CASEFOLD, "casefold" },
	{ F2FS_FEATURE_COMPRESSION, "compression" },
	{ F2FS_FEATURE_RO, "ro" },
	{ 0, 0 },
};
...
{
	__le32 f = sb->feature;
	struct sb_feature *sf;
	char msg[1024] = {0,};
	int i;

	sprintf(msg, "Info: superblock features = %x :", f);
	for (sf = sb_feature_list; sf->string; sf++) {
		if (f & cpu_to_le32(sf->mask))
			sprintf(msg + strlen(msg), " %s", sf->string);
	}
	sprintf(msg + strlen(msg), "\n");
	stlog(msg);
...
void update_superblock(struct f2fs_super_block *sb, int sb_mask)
{
	...
	for (addr = SB0_ADDR; addr < SB_MAX_ADDR; addr++) {
		/* read & update super_block to keep signing data */
...
out:
#endif
		/* P221221-04407: Force to check fully one time on T OS */
#define SEC_API_LEVEL_T_OS (33)
		if (sbi->raw_super->api_level != SEC_API_LEVEL_T_OS) {
			sbi->raw_super->api_level = (__u8)SEC_API_LEVEL_T_OS;
			c.auto_fix = 0;
			c.fix_on = 1;
			c.update_sb = true;
			stlog("Info: fsck run with full-check to fix compress released issue\n");
		}
...
static int tune_sb_features(struct f2fs_sb_info *sbi)
{
	...
	if (sb->feature & cpu_to_le32(F2FS_FEATURE_COMPRESSION))
		def_mnt_opts = "flush_group=5666,inlinecrypt,compress_log_size=5,gc_merge";
	else
		def_mnt_opts = "flush_group=5666,inlinecrypt,gc_merge";
